...
void Reassembler::cache_bytes( uint64_t first_index, string data, bool is_last_substring ){
  //找到与所给区间有关系的之前位于组装区的两个左右区间
  auto left = lower_bound(buffer_.begin(), buffer_.end(), first_index, [](auto &&elem, uint64_t
  index){
    return index > (get<0>(elem) + get<1>(elem).length());
  });
  auto right = upper_bound(left, buffer_.end(), first_index + data.length(), [](uint64_t index, auto &&
  elem){
